...
{
    //定义变量
    double u = 237, v, sigma , sigma0= 0.00001, best_x = 0, best_y = 0;
    static double x_1, x_2, y_1, y_2, a, x_0, y_0, temp1, tempcos, tempsin, deltay,deltax,tempdx, tempdy, dis,tempx1, tempx2, tempy1, tempy2;//!!!把尽可能多的变量挪到函数外面去!!!
    s32 time = 0, pretotal = 0, num = 2, test = 10;//改sigma0,num,test
    ...
    while (time < tot - num)
    {
        ...
        x_0 = (x_1 + x_2) / 2;
        y_0 = (y_1 + y_2) / 2;
        temp1 = sqrt(v - 0.25 * pow((dis), 2));
        tempcos = deltax/dis;
        tempsin = deltay/dis;
        tempdx = -1*temp1 * tempsin;
        tempdy = temp1 * tempcos;
        tempx1 = x_0 - tempdx;
        tempx2 = x_0 + tempdx;
        tempy1 = y_0 - tempdy;
        tempy2 = y_0 + tempdy;
        ...
        if (time >= test && time < tot - test)
        {
            ...
            {
                sigma = sigma0 * (pow(planeD[time][0], 2) + pow(planeD[time][1], 2));
